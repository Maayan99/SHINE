...
def create_mock_dataset() -> Tuple[List[str], List[str]]:
    texts = [
        ...
        "The Great Wall of China is an ancient architectural marvel.",
...
def load_checkpoint(model, metamodel, metanetwork, tokenizer, in_dir: str):
    ...
    tokenizer.from_pretrained(in_dir)
...
def main(cfg: DictConfig):
    ...
    if cfg.data.source == "mock":
        ...
        # Placeholder: switch to your real dataset construction when ready
        train_texts, val_texts = create_mock_dataset()
    ...
    def one_train_epoch(epoch):
            ...
            if step % max(1, cfg.run.gradient_accumulation_steps) == 0:
                ...
                if cfg.eval.eval_steps and global_step % cfg.eval.eval_steps == 0:
                    ...
                    if cfg.save.save_best and eval_metrics["eval_loss"] < best_eval_loss:
                        ...
                        print(f"New best model! Saving to {best_dir}")
                        save_checkpoint(
        ...
        print(f"Epoch {epoch} done. train_loss={epoch_avg:.4f} train_ppl={epoch_ppl:.2f}")

        eval_metrics = evaluate(model, metamodel, metanetwork, val_loader, device, use_amp=cfg.run.use_fp16)
        print(f"[Epoch {epoch} Eval] loss={eval_metrics['eval_loss']:.4f} ppl={eval_metrics['perplexity']:.2f}")
        if cfg.save.save_best and eval_metrics["eval_loss"] < best_eval_loss:
            best_eval_loss = eval_metrics["eval_loss"]
            best_dir = os.path.join(ckpt_root, "best")
            print(f"New best model! Saving to {best_dir}")
    ...
    # Also mirror to a stable path if requested (relative to original CWD)
    if cfg.paths.output_dir:
        # hydra changes CWD; map user path to absolute original location
        ...
        print(f"Model saved to {stable_out}")

    print(f"All artifacts in Hydra run dir: {hydra_run_dir}")
